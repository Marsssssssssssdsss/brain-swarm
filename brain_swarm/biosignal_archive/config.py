...
class NeuralSimConfig:
    ...
    duration: float = 1.0
    firing_rate_range: tuple = (0.5, 20.0)
...
class NeuromodConfig:
    """神经调控配置 (非侵入式"写入")"""
    enable: bool = True
    method: str = "tfus"  # tfus | tdcs | tacs
    target_state: str = "relax"  # relax | excite | suppress
    closed_loop: bool = True

    # tFUS 参数
    tfus: dict = field(default_factory=lambda: {
        "frequency": 500e3,
        "intensity": 1.0,
        "duty_cycle": 0.3,
    ...
    # tDCS 参数
    tdcs: dict = field(default_factory=lambda: {
        "current": 1.0,
        "electrode_size": 25,
    })
...
class PipelineConfig:
    ...
    neural_sim: NeuralSimConfig = field(default_factory=NeuralSimConfig)
